...
const HomeModule = ({ nextModule, unlockAchievement, gameState }: Pick<ModuleProps, 'nextModule' | 'unlockAchievement' | 'gameState'>) => (
...
const WelfareModule = ({ nextModule }: Pick<ModuleProps, 'nextModule'>) => {
    ...
    <ModuleLayout title="Що таке Громадський Добробут?" onNext={() => nextModule('realization')}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        <div className="glass-card">
          <h4 className="text-xl font-bold mb-4 flex items-center gap-2"><Heart className="text-pink-400" /> Якість життя</h4>
        ...
        <div className="glass-card">
          <h4 className="text-xl font-bold mb-4 flex items-center gap-2"><Star className="text-yellow-400" /> Спосіб життя</h4>
          <div className="space-y-4">
        ...
        {active && (
          ...
          >
            <h4 className="text-2xl font-bold text-white mb-4">
              ...
              {active === 'env' && "Приклад: Екологія та Душевне Дозвілля"}
...
const RealizationModule = ({ nextModule }: Pick<ModuleProps, 'nextModule'>) => {
  const opportunities = [
    { 
      id: '1', 
      ...
      impact: 'Подготовка майбутніх професіоналів та розвиток інтелекту громади.'
    },
    { 
      id: '2', 
        ...
        {opportunities.map(opt => (
          <div
            ...
            className={`p-6 rounded-2xl border backdrop-blur-md transition-all cursor-pointer ${
              selected === opt.id ? 'scale-[1.02] ring-2 ring-indigo-500 ' + opt.color.replace('20', '40') : 'bg-white/5 border-white/10 hover:bg-white/10 ' + opt.color.replace('bg-', 'text-').replace('text-', '')
            }`}
          >
...
const RolesModule = ({ addXP, nextModule }: Pick<ModuleProps, 'addXP' | 'nextModule'>) => {
  const roles = [
    { id: '1', title: 'Активний учасник', icon: Users, desc: 'Участь в учнівському самоврядуванні, шкільних заходах та толоках.' },
    { id: '2', title: 'Генератор ідей', icon: Lightbulb, desc: 'Пропонування нових проектів, гуртків чи способів навчання.' },
    { id: '3', title: 'Волонтер', icon: Heart, desc: 'Допомога тим, хто цього потребує, та турбота про довкілля громади.' },
    { id: '4', title: 'Амбасадор змін', icon: Globe, desc: 'Поширення знань та цінностей добробуту серед друзів та знайомих.' }
  ];

  const scenarios = [
    {
      title: "Ситуація 1",
      text: "Ти бачиш, як хтось ображає іншого учня в коридорі. Твоя реакція?",
      options: [
        { text: "Втрутитися і підтримати того, кого ображають", score: 50 },
        { text: "Повідомити вчителя або службу підтримки", score: 30 },
        { text: "Пройти повз, це не моя справа", score: 0 }
      ]
    },
    {
      title: "Ситуація 2",
      text: "У школі пропонують створити новий еко-гурток. Ти?",
      options: [
        { text: "Запропоную свою допомогу в організації", score: 50 },
        { text: "Просто приєднаюся як учасник", score: 20 },
        { text: "Це нудно, краще пограю в ігри", score: 0 }
      ]
    },
    {
      title: "Ситуація 3",
      text: "На твоєму подвір'ї лежить сміття. Твої дії?",
      options: [
        { text: "Зберу друзів і ми все приберемо", score: 50 },
        { text: "Просто підніму і викину сам", score: 30 },
        { text: "Поскаржуся в соцмережах і нічого не зроблю", score: 5 }
      ]
    }
  ];
...
const EducationModule = ({ nextModule }: Pick<ModuleProps, 'nextModule'>) => {
  const categories = [
    {
      id: 'dist',
...
const ImportanceModule = ({ nextModule }: Pick<ModuleProps, 'nextModule'>) => (
...
const TasksModule = ({ nextModule }: Pick<ModuleProps, 'nextModule'>) => {
  const tasks = [
    {
      id: 1,
...
const MillionaireModule = ({ addXP, unlockAchievement, nextModule }: Pick<ModuleProps, 'addXP' | 'unlockAchievement' | 'nextModule'>) => {
...
// --- Trajectory Module ---
const TrajectoryModule = ({ nextModule, unlockAchievement, completeModule, setCurrentModule }: Pick<ModuleProps, 'nextModule' | 'unlockAchievement' | 'completeModule' | 'setCurrentModule'>) => {
  const goals = [
...
export default function App() {
  ...
  const moduleRenderer = () => {
    switch (currentModule) {
      case 'home': return <HomeModule nextModule={nextModule} unlockAchievement={unlockAchievement} gameState={gameState} />;
      case 'welfare': return <WelfareModule nextModule={nextModule} />;
      case 'realization': return <RealizationModule nextModule={nextModule} />;
      case 'roles': return <RolesModule addXP={addXP} nextModule={nextModule} />;
      case 'education': return <EducationModule nextModule={nextModule} />;
      case 'importance': return <ImportanceModule nextModule={nextModule} />;
      case 'tasks': return <TasksModule nextModule={nextModule} />;
      case 'millionaire': return <MillionaireModule addXP={addXP} unlockAchievement={unlockAchievement} nextModule={nextModule} />;
      case 'trajectory': return <TrajectoryModule nextModule={nextModule} unlockAchievement={unlockAchievement} completeModule={completeModule} setCurrentModule={setCurrentModule} />;
      default: return <HomeModule nextModule={nextModule} unlockAchievement={unlockAchievement} gameState={gameState} />;
    }
  };
